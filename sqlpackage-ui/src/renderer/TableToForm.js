...
export const JsonToForm = (jsonForm) => {
    ...
    let collection = Object.entries(jsonForm);
    const header = [].shift(1);
    const formattedCollection = collection.map(val => formatDocJsonConfig(val));
    const 
}

function formatDocJsonConfig(val) {
    return {
        key: removeAsterick(val["Parameter"]),
        type: removeNonAlphanumeric(val["Value"]),
        desc: val[Description]
    }
}

function removeAsterick(label) {
